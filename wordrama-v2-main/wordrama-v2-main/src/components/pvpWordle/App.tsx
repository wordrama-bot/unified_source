...
import { GamepadIcon, PlusIcon, SettingsIcon } from 'lucide-react'
...
function App(){
  ...
  const { data: allTimeStats, isLoading: isLoadingAllTimeStats } = useGetMyAllTimeWordleStatsByGameModeQuery(gameState.gameMode);
  ...
  const gamesWon = allTimeStats?.data[`${wordPackAllTimeStatsMap[wordPack]}GamesWon`] || allTimeStats?.data?.gamesWon || 0;
  const gamesLost = allTimeStats?.data[`${wordPackAllTimeStatsMap[wordPack]}GamesLost`] || allTimeStats?.data?.gamesLost || 0;
  ...
  const { data: streakData, isLoading: isLoadingStreakData, isError: isErrorStreakData } = useGetWordleStreakQuery({ gameMode, wordPack });
  ...
  return (
    <Div100vh>
      ...
      <div className="flex h-full flex-col">
        ...
        <div className='grid grid-cols-2 mx-auto w-1/2 px-1 pt-20 pb-8 sm:px-6 md:max-w-7xl lg:px-8 short:pb-2 short:pt-2'>
          <Grid
            solution={isCustom ? gameState.custom.solution : gameState.modes[gameState.gameMode][gameState.wordPack].solution}
            guesses={isCustom ? gameState.custom.guessses : gameState.modes[gameState.gameMode][gameState.wordPack].guesses || []}
