...
import PlayerIdentityPanel from "@/components/admin/PlayerIdentityPanel";
...
export default function AdminPlayerProfilePage() {
  ...
  function handleRetryIdentityReport() {
    loadIdentity(playerId);
  }
          ...
          <div className="mt-6 space-y-4">
            <Panel title="Quick Actions">
              <div className="grid gap-3 sm:grid-cols-2">
                <ActionButton
                  label="Grant Coins"
                  onClick={() => setShowGrantCoins(true)}
                />
                ...
                <ActionButton
                  label="Unban Player"
                  onClick={() => setShowUnbanPlayer(true)}
                />

                <ActionButton label="Reset Streak" disabled />

                <ActionButton
                  label="Copy Player JSON"
                  onClick={() => copyPlayerJson(profile)}
                />
              ...
              <p className="mt-4 text-xs text-muted-foreground">
                Write actions are intentionally disabled until their backend
              ...
              {(profile.coinBalance ?? []).length === 0 ? (
                ...
                profile.coinBalance.map((row: any) => (
                  <div key={row.id} className="rounded-lg border p-3">
                    ...
                    <KeyValue
                      label="Available"
                      value={row.available_balance.toLocaleString()}
                    />

                    <KeyValue
                      label="Lifetime Earned"
                      value={row.lifetime_earned.toLocaleString()}
                    />

                    <KeyValue
                      label="Lifetime Spent"
                      value={row.lifetime_spent.toLocaleString()}
                    />
                  </div>
                ))
              )}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="rounded border p-3">
                          ...
                          <p className="text-2xl font-bold">
                            {Number(streak.best_streak).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-3">
                  {profile.activeBans.map((ban: any) => (
                    ...
                    >
                      ...
                      <KeyValue
                        label="Expires"
                        value={formatDate(ban.expires_at)}
                      />
                    </div>
                  ))}
                </div>
      ...
      {activeTab === "identity" && (
        <div className="mt-6">
          <PlayerIdentityPanel
            ...
            onRetry={handleRetryIdentityReport}
          />
        </div>
      )}
