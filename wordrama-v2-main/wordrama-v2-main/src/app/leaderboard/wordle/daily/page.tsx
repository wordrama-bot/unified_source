...
export default function WordleDailyLeaderboardPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  //getAppInsights().trackPageView({ name: 'Wordle Daily Leaderboard' });
  ...
  const WORD_PACKS: Record<string, {
    ...
    all: {
      ...
      prefix: "",
      columnPrefix: "",
      rankColumn: "daily_rank",
    },
    "4 letter": {
      label: "4 Letter",
      prefix: "fourLetter",
      columnPrefix: "four_letter",
      rankColumn: "daily_rank_4_letter",
    },
    "5 letter": {
      label: "5 Letter",
      prefix: "fiveLetter",
      columnPrefix: "five_letter",
      rankColumn: "daily_rank_5_letter",
    },
    "5 letter crazy": {
      label: "5 Letter Crazy",
      prefix: "fiveLetterCrazy",
      columnPrefix: "five_letter_crazy",
      rankColumn: "daily_rank_five_letter_crazy",
    },
    "6 letter": {
      label: "6 Letter",
      prefix: "sixLetter",
      columnPrefix: "six_letter",
      rankColumn: "daily_rank_6_letter",
    },
    "7 letter": {
      label: "7 Letter",
      prefix: "sevenLetter",
      columnPrefix: "seven_letter",
      rankColumn: "daily_rank_7_letter",
    },
    "8 letter": {
      label: "8 Letter",
      prefix: "eightLetter",
      columnPrefix: "eight_letter",
      rankColumn: "daily_rank_8_letter",
    },
    "9 letter": {
      label: "9 Letter",
      prefix: "nineLetter",
      columnPrefix: "nine_letter",
      rankColumn: "daily_rank_9_letter",
    },
    "10 letter": {
      label: "10 Letter",
      prefix: "tenLetter",
      columnPrefix: "ten_letter",
      rankColumn: "daily_rank_10_letter",
    },
    "11 letter": {
      label: "11 Letter",
      prefix: "elevenLetter",
  ...
  const { data: dailyData, isLoading: isLoadingDaily, error: dailyError } = useGetDailyWordleLeaderboardQuery({ page: Number(searchParams.get('page') || 1), orderBy: sortByKey });
  const { data: dailyTop3Data, isLoading: isLoadingTop3Daily, error: dailyTop3Error } = useGetDailyWordleLeaderboardQuery({ page: 1, orderBy: sortByKey });
  ...
  useEffect(() => {
    ...
    setLeaderboardData({ data: [] });
  }, [dailyData, dailyError, sortByKey]);
        ...
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
              ...
              href={`${window.location.pathname}?page=${dailyData?.metadata?.previousPage}`}
            />
          </PaginationItem>
          <PaginationItem>
            <PaginationLink className="text-text dark:text-darkText" href={`${window.location.pathname}?page=${dailyData?.metadata?.currentPage}`}>{ dailyData?.metadata?.currentPage }</PaginationLink>
          </PaginationItem>
          <PaginationItem className="text-white">
            <PaginationEllipsis className="text-text dark:text-darkText" />
          </PaginationItem>
          { dailyData?.metadata?.currentPage < dailyData?.metadata?.totalPages && (
            <>
              <PaginationItem>
                <PaginationLink className="text-text dark:text-darkText" href={`${window.location.pathname}?page=${dailyData?.metadata?.totalPages}`}>{ dailyData?.metadata?.totalPages }</PaginationLink>
              </PaginationItem>
              <PaginationItem>
                <PaginationNext
                  className="text-text dark:text-darkText"
                  href={`${window.location.pathname}?page=${dailyData?.metadata?.nextPage}`}
                />
              </PaginationItem>
            </>
          )}
    ...
    <div className="flex min-h-screen w-full flex-col">
      <main className="flex min-h-[calc(100vh_-_theme(spacing.16))] flex-1 flex-col gap-4 bg-muted/40 p-4 md:gap-8 md:p-10">
        <div className="mx-auto grid w-full max-w-6xl gap-2">
          <h1 className="text-3xl text-text dark:text-darkText font-semibold">Wordle | Daily Leaderboard</h1>
        </div>
        <Card className="w-full max-w-6xl mx-auto">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              { dailyTop3Data?.data?.slice(0, 3).map((player, index) => (
                    ...
                    <HoverCardTrigger asChild>
                      <CardContent className="pt-6">
                        ...
                        <div className="mt-2 text-sm text-muted-foreground">
                          ...
                          <p>Best Streak: {prefix ? player[`bestStreak${prefix}`] : player.dailyBestStreak}</p>
                        </div>
                      </CardContent>
                    </HoverCardTrigger>
                    ...
                    leaderboardData.data.map((entry, index) => (
                      <TableRow key={entry?.id}>
                        <TableCell className="font-medium">
                          { sortBy === 'rank' ?
                            (numberPrefix ? entry[`dailyRank_${numberPrefix}`] : entry.dailyRank) :
                            entry.position
                          }
                        </TableCell>
                        <TableCell>
