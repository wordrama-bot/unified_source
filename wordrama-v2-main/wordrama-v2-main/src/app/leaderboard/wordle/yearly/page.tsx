...
export default function WordleYearlyLeaderboardPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  //getAppInsights().trackPageView({ name: 'Wordle Yearly Leaderboard' });
  ...
  const { data: yearlyData, isLoading: isLoadingYearly, error: yearlyError } = useGetYearlyWordleLeaderboardQuery({ page: Number(searchParams.get('page') || 1), orderBy: sortByKey });
  const { data: yearlyTop3Data, isLoading: isLoadingTop3Yearly, error: yearlyTop3Error } = useGetYearlyWordleLeaderboardQuery({ page: 1, orderBy: sortByKey });
  ...
  useEffect(() => {
    ...
    if (yearlyData?.data) {
      setLeaderboardData(yearlyData);
      return;
    }

    setLeaderboardData({ data: [] });
  }, [yearlyData, yearlyError, sortByKey]);
        ...
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
              ...
              href={`${window.location.pathname}?page=${yearlyData?.metadata?.previousPage}`}
            />
          </PaginationItem>
          <PaginationItem>
            <PaginationLink className="text-text dark:text-darkText" href={`${window.location.pathname}?page=${yearlyData?.metadata?.currentPage}`}>{ yearlyData?.metadata?.currentPage }</PaginationLink>
          </PaginationItem>
          <PaginationItem className="text-white">
            <PaginationEllipsis className="text-text dark:text-darkText" />
          </PaginationItem>
          { yearlyData?.metadata?.currentPage < yearlyData?.metadata?.totalPages && (
            <>
              <PaginationItem>
                <PaginationLink className="text-text dark:text-darkText" href={`${window.location.pathname}?page=${yearlyData?.metadata?.totalPages}`}>{ yearlyData?.metadata?.totalPages }</PaginationLink>
              </PaginationItem>
              <PaginationItem>
                <PaginationNext
                  className="text-text dark:text-darkText"
                  href={`${window.location.pathname}?page=${yearlyData?.metadata?.nextPage}`}
                />
              </PaginationItem>
            </>
          )}
    ...
    <div className="flex min-h-screen w-full flex-col">
      <main className="flex min-h-[calc(100vh_-_theme(spacing.16))] flex-1 flex-col gap-4 bg-muted/40 p-4 md:gap-8 md:p-10">
        <div className="mx-auto grid w-full max-w-6xl gap-2">
          <h1 className="text-3xl text-text dark:text-darkText font-semibold">Wordle | { new Date().getFullYear() } Leaderboard</h1>
        </div>
        <Card className="w-full max-w-6xl mx-auto">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              { yearlyTop3Data?.data?.slice(0, 3).map((player, index) => (
                    ...
                    <HoverCardTrigger asChild>
                      <CardContent className="pt-6">
                        ...
                        <div className="mt-2 text-sm text-muted-foreground">
                          ...
                          <p>Best Streak: {prefix ? player[`bestStreak${prefix}`] : player.yearlyBestStreak}</p>
                        </div>
                      </CardContent>
                    </HoverCardTrigger>
                    ...
                    leaderboardData.data.map((entry, index) => (
                      <TableRow key={entry?.id}>
                        <TableCell className="font-medium">
                          { sortBy === 'rank' ?
                            (numberPrefix ? entry[`yearlyRank_${numberPrefix}`] : entry.yearlyRank) :
                            entry.position
                          }
                        </TableCell>
                        <TableCell>
