import type { CatalogItem } from '../../types/marketplaceV2'

export const CATALOG: CatalogItem[] = [
  {
    catalogItemId: 'ba8671aa-7481-43e5-a1ac-f2b73433a315',
    sku: 'WP-4',
    itemType: 'WORD_PACK',
    itemName: '4 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_4_letter',
    entitlementKey: 'WORD_PACK:FOUR_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '7ed7d3a6-3d07-4603-b9cf-6a6e717b9d1f',
    sku: 'WP-5-CRAZY',
    itemType: 'WORD_PACK',
    itemName: '5 Letter Crazy',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_5_letter_crazy',
    entitlementKey: 'WORD_PACK:FIVE_LETTER_CRAZY',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'b1b96d0e-5b1a-403e-80be-88f3d2bae873',
    sku: 'WP-11-EXP',
    itemType: 'WORD_PACK',
    itemName: '11 Letter Expansion',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_11_letter_expansion',
    entitlementKey: 'WORD_PACK:ELEVEN_LETTER_EXPANSION',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '7f06b10e-d52a-4ae3-b77f-a7e9a7c5e5fb',
    sku: 'WP-12',
    itemType: 'WORD_PACK',
    itemName: '12 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_12_letter',
    entitlementKey: 'WORD_PACK:TWELVE_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'b8c73f14-79ad-4495-9fd9-a4be65d5fcbc',
    sku: 'WP-13',
    itemType: 'WORD_PACK',
    itemName: '13 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_13_letter',
    entitlementKey: 'WORD_PACK:THIRTEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '3159552d-8c96-4bb5-aafa-ebf36aa5a2c2',
    sku: 'WP-14',
    itemType: 'WORD_PACK',
    itemName: '14 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_14_letter',
    entitlementKey: 'WORD_PACK:FOURTEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'fef67eba-96db-4f5e-8b25-81487a1dbc9d',
    sku: 'WP-15',
    itemType: 'WORD_PACK',
    itemName: '15 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_15_letter',
    entitlementKey: 'WORD_PACK:FIFTEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '1ee2de50-072f-4718-b8ac-7663f3069f2e',
    sku: 'WP-16',
    itemType: 'WORD_PACK',
    itemName: '16 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_16_letter',
    entitlementKey: 'WORD_PACK:SIXTEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '80e197a9-0829-4074-8e85-a88e6e8b7ea0',
    sku: 'WP-17',
    itemType: 'WORD_PACK',
    itemName: '17 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_17_letter',
    entitlementKey: 'WORD_PACK:SEVENTEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '425c96ab-beff-40ef-9774-feb6db135644',
    sku: 'WP-18',
    itemType: 'WORD_PACK',
    itemName: '18 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_18_letter',
    entitlementKey: 'WORD_PACK:EIGHTEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '1d348c05-c51e-4ea3-a888-d4823436704f',
    sku: 'WP-19',
    itemType: 'WORD_PACK',
    itemName: '19 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_19_letter',
    entitlementKey: 'WORD_PACK:NINETEEN_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '6e66a620-8e17-4f75-aa0b-1c282aafb9d8',
    sku: 'WP-20',
    itemType: 'WORD_PACK',
    itemName: '20 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_20_letter',
    entitlementKey: 'WORD_PACK:TWENTY_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '72215e5b-6638-4388-84bc-55dcd36c0e05',
    sku: 'WP-21',
    itemType: 'WORD_PACK',
    itemName: '21 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_21_letter',
    entitlementKey: 'WORD_PACK:TWENTYONE_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'db526774-11da-47de-b410-5b47a4168db8',
    sku: 'WP-22',
    itemType: 'WORD_PACK',
    itemName: '22 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_22_letter',
    entitlementKey: 'WORD_PACK:TWENTYTWO_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'ab14511c-f2ac-4b16-a8ef-7cb8ed61a2cc',
    sku: 'WP-23',
    itemType: 'WORD_PACK',
    itemName: '23 Letter Pack',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'wp_23_letter',
    entitlementKey: 'WORD_PACK:TWENTYTHREE_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '3d3ff93b-65c1-4d36-902e-3a889c71ac86',
    sku: 'WP-12-23',
    itemType: 'WORD_PACK',
    itemName: '12-23 Letter Mega Pack',
    priceCoins: 10000,
    priceUsdCents: 999,
    stripePriceEnvVar: 'wp_12_to_23_letter',
    entitlementKey: 'WORD_PACK:TWELVE_TO_TWENTYTHREE_LETTER',
    entitlementType: 'WORD_PACK',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'eea2ee39-7ff5-4cfc-906b-608d01555afa',
    sku: 'THEME-CYBERPUNK',
    itemType: 'THEME',
    itemName: 'Cyberpunk Theme',
    image: '/images/themes/cyberpunk.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_cyberpunk',
    entitlementKey: 'THEME:CYBERPUNK',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'c42e21b8-e3e2-487b-9307-698063f29dcd',
    sku: 'THEME-MIAMI-VICE',
    itemType: 'THEME',
    itemName: 'Miami Vice Theme',
    image: '/images/themes/miami-vice.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_miami_vice',
    entitlementKey: 'THEME:MIAMI_VICE',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'db984ab5-f0a9-43a9-9800-702150de76f5',
    sku: 'THEME-GALAXY',
    itemType: 'THEME',
    itemName: 'Galaxy Theme',
    image: '/images/themes/galaxy.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_galaxy',
    entitlementKey: 'THEME:GALAXY',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '8642cdb8-7f8e-4c58-be01-3f9d5362326d',
    sku: 'THEME-ICE',
    itemType: 'THEME',
    itemName: 'Ice Theme',
    image: '/images/themes/ice.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_ice',
    entitlementKey: 'THEME:ICE',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '4085cec0-6c84-4732-b70c-a80938afe0a5',
    sku: 'THEME-LAVA',
    itemType: 'THEME',
    itemName: 'Lava Theme',
    image: '/images/themes/lava.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_lava',
    entitlementKey: 'THEME:LAVA',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'a0144a0f-6867-4140-a520-f64375f35990',
    sku: 'THEME-FOREST',
    itemType: 'THEME',
    itemName: 'Forest Theme',
    image: '/images/themes/forest.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_forest',
    entitlementKey: 'THEME:FOREST',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '91d60b4a-3dfa-4c90-8992-fae1e2bed0c9',
    sku: 'THEME-CABIN',
    itemType: 'THEME',
    itemName: 'Cabin Theme',
    image: '/images/themes/cabin.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_cabin',
    entitlementKey: 'THEME:CABIN',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '7855a094-b1df-49c5-abf0-622e651df69a',
    sku: 'THEME-PIRATE',
    itemType: 'THEME',
    itemName: 'Pirate Theme',
    image: '/images/themes/pirate.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_pirate',
    entitlementKey: 'THEME:PIRATE',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '49a35b50-db84-4854-847e-e9610874f878',
    sku: 'THEME-ARCADE',
    itemType: 'THEME',
    itemName: 'Arcade Theme',
    image: '/images/themes/arcade.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_arcade',
    entitlementKey: 'THEME:ARCADE',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'c58ed56e-3891-44c6-b195-b9d1c8bbe980',
    sku: 'THEME-SYNTHWAVE',
    itemType: 'THEME',
    itemName: 'Synthwave Theme',
    image: '/images/themes/synthwave.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_synthwave',
    entitlementKey: 'THEME:SYNTHWAVE',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '147ed2fa-7a53-45f1-8721-60e2705dcda2',
    sku: 'THEME-EMPIRE',
    itemType: 'THEME',
    itemName: 'Empire Theme',
    image: '/images/themes/empire.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_empire',
    entitlementKey: 'THEME:EMPIRE',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '0f291c62-c811-436d-96cc-cf92a8f240e5',
    sku: 'THEME-MATRIX',
    itemType: 'THEME',
    itemName: 'Matrix Theme',
    image: '/images/themes/matrix.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'theme_matrix',
    entitlementKey: 'THEME:MATRIX',
    entitlementType: 'THEME',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: '5a9444bc-04c0-471e-a953-3c5b73b8c001',
    sku: 'AVATAR-FRAME-BRONZE',
    itemType: 'AVATAR',
    itemName: 'Bronze Frame',
    image: '/images/avatars/frames/bronze.png',
    priceCoins: 1000,
    priceUsdCents: 99,
    stripePriceEnvVar: 'avatar_frame_bronze',
    entitlementKey: 'FRAME:BRONZE',
    entitlementType: 'AVATAR',
    isPurchasable: true,
    isStripePurchasable: true,
  },
  {
    catalogItemId: 'avatar-crown-alltime-champion',
    sku: 'AVATAR:CROWN_ALLTIME_CHAMPION',
    itemType: 'AVATAR',
    itemName: 'Crown of Champions',
    priceCoins: 0,
    entitlementKey: 'AVATAR:CROWN_ALLTIME_CHAMPION',
    entitlementType: 'AVATAR_STYLE',
    isPurchasable: false,
    isStripePurchasable: false,
  },
  {
    catalogItemId: 'avatar-crown-daily-champion',
    sku: 'AVATAR:CROWN_DAILY_CHAMPION',
    itemType: 'AVATAR',
    itemName: 'Daily Champion Crown',
    priceCoins: 0,
    entitlementKey: 'AVATAR:CROWN_DAILY_CHAMPION',
    entitlementType: 'AVATAR_STYLE',
    isPurchasable: false,
    isStripePurchasable: false,
  },
  {
    catalogItemId: 'avatar-crown-weekly-champion',
    sku: 'AVATAR:CROWN_WEEKLY_CHAMPION',
    itemType: 'AVATAR',
    itemName: 'Weekly Champion Crown',
    priceCoins: 0,
    entitlementKey: 'AVATAR:CROWN_WEEKLY_CHAMPION',
    entitlementType: 'AVATAR_STYLE',
    isPurchasable: false,
    isStripePurchasable: false,
  },
  {
    catalogItemId: 'avatar-crown-monthly-champion',
    sku: 'AVATAR:CROWN_MONTHLY_CHAMPION',
    itemType: 'AVATAR',
    itemName: 'Monthly Champion Crown',
    priceCoins: 0,
    entitlementKey: 'AVATAR:CROWN_MONTHLY_CHAMPION',
    entitlementType: 'AVATAR_STYLE',
    isPurchasable: false,
    isStripePurchasable: false,
  },
  {
    catalogItemId: 'avatar-crown-yearly-champion',
    sku: 'AVATAR:CROWN_YEARLY_CHAMPION',
    itemType: 'AVATAR',
    itemName: 'Yearly Champion Crown',
    priceCoins: 0,
    entitlementKey: 'AVATAR:CROWN_YEARLY_CHAMPION',
    entitlementType: 'AVATAR_STYLE',
    isPurchasable: false,
    isStripePurchasable: false,
  },
]

export function getCatalogItemsByIds(ids: string[]): CatalogItem[] {
  const map = new Map(CATALOG.map(item => [item.catalogItemId, item]))
  return ids.map(id => map.get(id)).filter(Boolean) as CatalogItem[]
}

export function getCatalogItems(): CatalogItem[] {
  return CATALOG;
}
